...
export default function CourseCard({ course, onEnroll, showEnrollButton = false, showProgress = false }: CourseCardProps) {
  ...
  return (
    <div className="course-card bg-white rounded-lg shadow border border-gray-100 overflow-hidden dark:bg-gray-800 dark:border-gray-700 hover:shadow-lg transition-shadow duration-300 h-full flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-grow">
        <Link href={`/courses/${course.id}`}>
          <h3 className="font-bold text-gray-900 dark:text-gray-100 mb-2 cursor-pointer hover:text-primary-600 dark:hover:text-primary-400 line-clamp-2">{title}</h3>
        </Link>
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4 line-clamp-3 flex-grow">{description}</p>
